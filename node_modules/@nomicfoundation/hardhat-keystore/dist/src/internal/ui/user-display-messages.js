...
export class UserDisplayMessages {
    static displayInvalidKeyErrorMessage(key) {
        return styleText("red", `Invalid value for key: "${key}". Keys can only have alphanumeric characters and underscores, and they cannot start with a number.`);
    }
    static displayKeyAlreadyExistsWarning(key, dev) {
        return (styleText("yellow", `The key "${key}" already exists in the ${getKeystoreType(dev)} keystore. Use the `) +
    ...
    static displayKeyNotFoundErrorMessage(key, dev) {
        return styleText("red", `Key "${key}" not found in the ${getKeystoreType(dev)} keystore`);
    }
